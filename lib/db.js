...
export async function deleteUser(userId) {
  ...
  await db.collection('user_progress').deleteMany({ user_id: userId.toString() });
  await db.collection('user_profiles').deleteOne({ userId: userId.toString() });
  await db.collection('roadmap_progress').deleteMany({ userId: userId.toString() });
...
export async function getAdminDashboardStats() {
  ...
  const totalRoadmaps = await db.collection('roadmaps').countDocuments({ published: true });
  const totalNodes = await db.collection('roadmap_nodes').countDocuments({ published: true });

  const activeLearners = await db.collection('roadmap_progress').distinct('userId', {
...
export async function logAdminAction(adminId, action, resourceType, resourceId, changes = null) {
  const { db } = await connectToDatabase();
...
export async function getUserDetailedInfo(userId) {
  ...
  const [profile, roadmapProgress, quizResults, quizAttempts] = await Promise.all([
    db.collection('user_profiles').findOne({ userId: userIdStr }),
    db.collection('roadmap_progress').find({ userId: userIdStr }).toArray(),
    db.collection('quiz_results').find({ userId: userIdStr }).sort({ completedAt: -1 }).toArray(),
...
export async function getUserActivityTimeline(userId, limit = 50) {
  ...
  const [quizResults, roadmapProgress, certificates, profileUpdates] = await Promise.all([
    db.collection('quiz_results')
      .find({ userId: userIdStr })
      .sort({ completedAt: -1 })
      .limit(limit)
      .toArray(),
    db.collection('roadmap_progress')
      .find({ userId: userIdStr })
      .sort({ updatedAt: -1 })
      .limit(limit)
      .toArray(),
...
export async function bulkDeleteUsers(userIds) {
  ...
  await Promise.all([
    ...
    db.collection('roadmap_progress').deleteMany({ userId: { $in: userIdStrs } }),
    db.collection('quiz_results').deleteMany({ userId: { $in: userIdStrs } }),
    db.collection('user_quiz_attempts').deleteMany({ userId: { $in: userIdStrs } })
  ]);
...
export async function getMentorshipStats() {
  ...
  const [total, pending, replied, closed, byType] = await Promise.all([
    ...
    db.collection('mentorship_requests').aggregate([
...
export async function getActiveUsersData(days = 30) {
  ...
  return Object.entries(byDate).map(([date, users]) => ({
    date,
    count: users.size
  })).sort((a, b) => new Date(a.date) - new Date(b.date));
}
...
export async function getAnalyticsOverview() {
  ...
  const [
    ...
    db.collection('roadmap_progress').distinct('userId', {
...
export async function getUserRetentionData() {
  ...
  const [totalUsers, active7Days, active30Days] = await Promise.all([
    db.collection('users').countDocuments(),
    db.collection('users').countDocuments({ lastLogin: { $gte: sevenDaysAgo } }),
    db.collection('users').countDocuments({ lastLogin: { $gte: thirtyDaysAgo } })
  ]);
  ...
  return {
    ...
    active7Days,
    active30Days,
    retention7Days: totalUsers > 0 ? Math.round((active7Days / totalUsers) * 100) : 0,
    retention30Days: totalUsers > 0 ? Math.round((active30Days / totalUsers) * 100) : 0
  };
}
...
export async function getTopLearners(limit = 10) {
  ...
  const topLearners = await db.collection('users')
    .aggregate([
      {
        $lookup: {
          from: 'quizzes',
...
export async function getChurnRiskUsers() {
  ...
  const churnRisk = await db.collection('users')
    .find({
...
export async function getUserGeographicDistribution() {
  ...
  const distribution = await db.collection('users')
    .aggregate([
      {
        $group: {
          _id: '$location',
...
export async function getQuizCompletionRates() {
  ...
  const stats = await db.collection('quizzes')
    .aggregate([
      {
        $group: {
          _id: '$roadmapId',
...
export async function getWeakTopicsAnalysis() {
  ...
  const weakTopics = await db.collection('quizzes')
    .aggregate([
      { $match: { score: { $lt: 70 } } },
      {
        $lookup: {
...
export async function getQuizTimeAnalysis() {
  ...
  const timeStats = await db.collection('quizzes')
    .aggregate([
      {
        $match: {
...
export async function getAttemptDistribution() {
  ...
  const distribution = await db.collection('quizzes')
    .aggregate([
      {
        $group: {
...
export async function getMentorshipRequestVolume(days = 30) {
  ...
  const volume = await db.collection('mentorship_requests')
    .aggregate([
...
export async function getMentorshipResponseTimeStats() {
  ...
  const stats = await db.collection('mentorship_requests')
    .aggregate([
      {
        $match: {
...
export async function getMentorshipCommonTopics() {
  ...
  const topics = await db.collection('mentorship_requests')
    .aggregate([
      {
        $group: {
          _id: '$requestType',
  ...
  return topics.map(topic => ({
    ...
    count: topic.count,
    avgResponseTime: topic.avgResponseTime ? Math.round(topic.avgResponseTime * 10) / 10 : null
  }));
}
...
export async function getMentorshipResolutionRate() {
  ...
  return {
    ...
    replied,
    pending,
    resolutionRate: total > 0 ? Math.round(((resolved + replied) / total) * 100) : 0
  };
}
...
export async function getMentorshipPeakTimes() {
  ...
  const requests = await db.collection('mentorship_requests')
    .find()
    .toArray();
