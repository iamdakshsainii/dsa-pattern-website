...
export async function getUserNotesForQuestion(userId, questionId) {
  const { db } = await connectToDatabase();
  const notes = await db
    .collection("notes")
    .find({ user_id: userId, question_id: questionId })
    .sort({ updated_at: -1 })
    .toArray();
  return serializeDocs(notes);
}

export async function createNote(userId, questionId, title, content) {
  const { db } = await connectToDatabase();
...
export async function getMasterRoadmaps() {
  const { db } = await connectToDatabase();
  const masters = await db.collection('master_roadmaps')
    .find({ published: true })
    .sort({ order: 1 })
    .toArray();
  return serializeDocs(masters);
}

export async function getMasterRoadmap(masterId) {
  const { db } = await connectToDatabase();
  const master = await db.collection('master_roadmaps').findOne({ masterId });
  return serializeDoc(master);
}

export async function getUserMasterProgress(userId, masterId) {
  const { db } = await connectToDatabase();
  const progress = await db.collection('master_roadmap_progress').findOne({
    userId: userId.toString(),
    masterId
  });
  return serializeDoc(progress);
...
export async function initializeMasterProgress(userId, masterId, academicInfo = {}) {
  const { db } = await connectToDatabase();

  const master = await getMasterRoadmap(masterId);
  if (!master) throw new Error('Master roadmap not found');

  const yearProgress = master.years.map(year => ({
    year: year.yearNumber,
    completionPercent: 0,
    completedRoadmaps: [],
    testOut: null,
    startedAt: year.yearNumber === 1 ? new Date() : null,
    completedAt: null
  }));
  ...
  const progress = {
    ...
    currentYear: 1,
    yearProgress,
    chosenTechStack: null,
    techStackProgress: null,
    academicInfo: {
      college: academicInfo.college || null,
      currentSemester: academicInfo.currentSemester || null,
      expectedGraduation: academicInfo.expectedGraduation || null,
      examDates: academicInfo.examDates || []
    },
    startedAt: new Date(),
    lastAccessedAt: new Date(),
    createdAt: new Date(),
  ...
  await db.collection('master_roadmap_progress').insertOne(progress);
  return progress;
}

export async function calculateYearProgress(userId, masterId, yearNumber) {
  try {
    const { db } = await connectToDatabase();
    ...
    const year = master.years.find(y => y.yearNumber === yearNumber);
    if (!year) return 0;
    ...
    if (roadmapSlugs.length === 0) return 0;

    const result = await db.collection('roadmap_progress').aggregate([
      {
        $match: {
          userId: userId.toString(),
          roadmapId: { $in: roadmapSlugs }
        }
      },
      {
        $group: {
    ...
    return Math.round(result[0].avgProgress || 0);
  } catch (error) {
    console.error('Error calculating year progress:', error);
    const progress = await getUserMasterProgress(userId, masterId);
    const yearProg = progress?.yearProgress?.find(y => y.year === yearNumber);
    return yearProg?.completionPercent || 0;
  }
}

export async function isYearUnlocked(userId, masterId, yearNumber) {
  ...
  if (progress?.yearProgress && Array.isArray(progress.yearProgress)) {
    const prevYear = progress.yearProgress.find(yp => yp.year === yearNumber - 1);
    if (prevYear) {
      return prevYear.completionPercent >= year.requiredProgress;
    }
  }

  const previousYearProgress = await calculateYearProgress(userId, masterId, yearNumber - 1);
  return previousYearProgress >= year.requiredProgress;
}

export async function saveTechStackChoice(userId, masterId, techStackSlug) {
  const { db } = await connectToDatabase();

  const result = await db.collection('master_roadmap_progress').updateOne(
    { userId: userId.toString(), masterId },
    {
      $set: {
...
export async function getTechStackRoadmaps() {
  const { db } = await connectToDatabase();

  const roadmaps = await db.collection('roadmaps').find({
...
export async function updateAcademicInfo(userId, masterId, academicInfo) {
  const { db } = await connectToDatabase();
...
export async function shouldActivateExamMode(userId, masterId) {
  const progress = await getUserMasterProgress(userId, masterId);
  if (!progress || !progress.academicInfo) return false;

  const { examDates } = progress.academicInfo;
  if (!examDates || examDates.length === 0) return false;
...
export async function updateMasterProgress(userId, masterId) {
  const { db } = await connectToDatabase();
  ...
  const updatedYearProgress = await Promise.all(
    master.years.map(async (year) => {
      const yearProg = progress.yearProgress.find(yp => yp.year === year.yearNumber);
      const completionPercent = await calculateYearProgress(userId, masterId, year.yearNumber);
      ...
      for (const slug of roadmapSlugs) {
        const roadmapProg = await getUserRoadmapProgress(userId.toString(), slug);
        if (roadmapProg && roadmapProg.overallProgress === 100) {
          completedRoadmaps.push(slug);
        }
      }
      ...
      return {
        ...
        completionPercent,
        completedRoadmaps,
        testOut: yearProg?.testOut || null,
        startedAt: yearProg?.startedAt || null,
        completedAt: completionPercent === 100 ? new Date() : yearProg?.completedAt
      };
    })
  );

  const currentYear = updatedYearProgress.findIndex(yp => yp.completionPercent < 100) + 1 || master.years.length;
...
export async function getYearTestQuestions(masterId, yearNumber, count = 10) {
  const { db } = await connectToDatabase();

  const allQuestions = await db.collection('year_test_questions')
    .find({ masterId, yearNumber })
    .toArray();
  ...
  return serializeDocs(selected);
}

export async function submitYearTest(userId, masterId, yearNumber, answers) {
  const { db } = await connectToDatabase();
  const { ObjectId } = await import("mongodb");

  const questionIds = answers.map(a => a.questionId);
  const questions = await db.collection('year_test_questions')
  ...
  const master = await getMasterRoadmap(masterId);
  const year = master.years.find(y => y.yearNumber === yearNumber);
  const threshold = year?.testOutThreshold || 80;
  const passed = percentage >= threshold;

  const previousAttempts = await db.collection('year_test_attempts')
    .countDocuments({ userId: userId.toString(), masterId, yearNumber });

  await db.collection('year_test_attempts').insertOne({
    userId: userId.toString(),
    masterId,
    yearNumber,
    attemptNumber: previousAttempts + 1,
  ...
  if (passed) {
    const progress = await getUserMasterProgress(userId, masterId);
    if (progress) {
      const updatedYearProgress = progress.yearProgress.map(yp => {
      ...
      await db.collection('master_roadmap_progress').updateOne(
        { userId: userId.toString(), masterId },
        {
          $set: {
            yearProgress: updatedYearProgress,
            currentYear: Math.min(yearNumber + 1, master.years.length),
            updatedAt: new Date()
          }
        }
      );
...
export async function getUserTestAttempts(userId, masterId, yearNumber) {
  const { db } = await connectToDatabase();

  const attempts = await db.collection('year_test_attempts')
    .find({ userId: userId.toString(), masterId, yearNumber })
    .sort({ submittedAt: -1 })
    .toArray();

  return serializeDocs(attempts);
}

export async function createTestQuestion(questionData) {
  const { db } = await connectToDatabase();

  const result = await db.collection('year_test_questions').insertOne({
    ...questionData,
    createdAt: new Date(),
    updatedAt: new Date()
  });

  return { success: true, questionId: result.insertedId.toString() };
}

export async function updateTestQuestion(questionId, questionData) {
  const { db } = await connectToDatabase();
  const { ObjectId } = await import("mongodb");

  const result = await db.collection('year_test_questions').updateOne(
    { _id: new ObjectId(questionId) },
    {
      $set: {
        ...questionData,
...
export async function deleteTestQuestion(questionId) {
  ...
  const result = await db.collection('year_test_questions').deleteOne({
    _id: new ObjectId(questionId)
  });
...
export async function getAllTestQuestions(masterId, yearNumber) {
  const { db } = await connectToDatabase();
...
export async function getMasterRoadmapAnalytics(masterId) {
  const { db } = await connectToDatabase();

  const totalUsers = await db.collection('master_roadmap_progress')
    .countDocuments({ masterId });

  const yearDistribution = await db.collection('master_roadmap_progress')
    .aggregate([
      { $match: { masterId } },
  ...
  const techStackChoices = await db.collection('master_roadmap_progress')
    .aggregate([
      { $match: { masterId, chosenTechStack: { $ne: null } } },
      { $group: { _id: '$chosenTechStack', count: { $sum: 1 } } }
    ]).toArray();
...
export async function getAcademicYears() {
  const { db } = await connectToDatabase()
  const years = await db.collection('academic_resources')
    .aggregate([
      { $group: { _id: '$year' } },
      { $sort: { _id: 1 } }
    ])
...
export async function getAcademicSemesters(year) {
  const { db } = await connectToDatabase()
  const semesters = await db.collection('academic_resources')
    .aggregate([
      { $match: { year } },
      { $group: { _id: '$semester' } },
      { $sort: { _id: 1 } }
...
export async function getAcademicSubjects(year, semester) {
  const { db } = await connectToDatabase()
  const subjects = await db.collection('academic_resources')
    .find({ year, semester })
    .sort({ subject: 1 })
    .toArray()
  return serializeDocs(subjects)
}

export async function getAcademicSubject(year, semester, subject) {
  const { db } = await connectToDatabase()
  const data = await db.collection('academic_resources').findOne({
    year,
    semester,
    subject
  })
...
export async function searchAcademicResources(query) {
  const { db } = await connectToDatabase()
  const results = await db.collection('academic_resources')
    .find({
      $or: [
        { subject: { $regex: query, $options: 'i' } },
        { 'resources.title': { $regex: query, $options: 'i' } }
      ]
    })
    .toArray()
  return serializeDocs(results)
...
export async function createAcademicSubject(data) {
  const { db } = await connectToDatabase()
  const result = await db.collection('academic_resources').insertOne({
    ...data,
    createdAt: new Date(),
    updatedAt: new Date()
  })
...
export async function updateAcademicSubject(year, semester, subject, data) {
  const { db } = await connectToDatabase()
  const result = await db.collection('academic_resources').updateOne(
...
export async function deleteAcademicSubject(year, semester, subject) {
  const { db } = await connectToDatabase()
  const result = await db.collection('academic_resources').deleteOne({
    year,
    semester,
    subject
  })
...
export async function addAcademicResource(year, semester, subject, resource) {
  const { db } = await connectToDatabase()
  const result = await db.collection('academic_resources').updateOne(
    { year, semester, subject },
    {
...
export async function deleteAcademicResource(year, semester, subject, resourceIndex) {
  const { db } = await connectToDatabase()
  const result = await db.collection('academic_resources').updateOne(
    { year, semester, subject },
    {
      $unset: { [`resources.${resourceIndex}`]: 1 },
      $set: { updatedAt: new Date() }
    }
  )

  // Remove null values
  await db.collection('academic_resources').updateOne(
    { year, semester, subject },
    { $pull: { resources: null } }
  )
...
export async function getAcademicsAnalytics() {
  ...
  const totalSubjects = await db.collection('academic_resources').countDocuments({})

  const byYear = await db.collection('academic_resources')
    .aggregate([
  ...
  const totalResources = await db.collection('academic_resources')
    .aggregate([
      { $group: { _id: null, total: { $sum: { $size: '$resources' } } } }
    ])
    .toArray()

  return {
    totalSubjects,
    totalResources: totalResources[0]?.total || 0,
    byYear: byYear.reduce((acc, y) => {
      acc[`year${y._id}`] = y.count
      return acc
    }, {})
  }
}
