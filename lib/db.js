...
export async function getUserMasterProgress(userId, masterId) {
  const { db } = await connectToDatabase();
  const progress = await db.collection('master_progress').findOne({
    userId: userId.toString(),
    masterId
  });
  return serializeDoc(progress);
...
export async function initializeMasterProgress(userId, masterId, academicInfo = {}) {
  ...
  await db.collection('master_progress').insertOne(progress);
  return progress;
}
...
export async function getMasterRoadmapAnalytics(masterId) {
  ...
  const totalUsers = await db.collection('master_progress')
    .countDocuments({ masterId });

  const yearDistribution = await db.collection('master_progress')
    .aggregate([
      { $match: { masterId } },
  ...
  const techStackChoices = await db.collection('master_progress')
    .aggregate([
