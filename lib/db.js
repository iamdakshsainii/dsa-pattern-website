...
export async function saveQuizResult(userId, roadmapId, score, answers, timeTaken) {
  const { db } = await connectToDatabase()
  ...
  const previousAttempts = await db.collection('quiz_results')
    .countDocuments({ userId, roadmapId })

  const validatedAnswers = Array.isArray(answers) ? answers.map((answer, index) => {
    // Handle both object and primitive answers
    ...
    return {
      ...
      correctAnswers: answer.correctAnswers || [answer.correctAnswer || answer.correct || ''],
...
export async function getQuizConfig(roadmapId) {
  ...
  const customQuiz = await db.collection('custom_quizzes')
    .findOne({ roadmapId })
...
export async function saveCustomQuiz(roadmapId, quizData) {
  ...
  return { success: true, modified: result.modifiedCount || result.upsertedCount }
}
...
export async function getAllQuizzesForAdmin() {
  ...
  const customQuizzes = await db.collection('custom_quizzes')
    .find({})
    .toArray()
...
export async function getUserActivityDashboard(userId) {
  ...
  const roadmapProgress = await db.collection('roadmap_progress')
    .find({ userId: userIdStr })
    .sort({ lastAccessedAt: -1 })
    .toArray()
  ...
  const enrichedProgress = await Promise.all(
    roadmapProgress.map(async (progress) => {
      ...
      return {
        ...
        completedNodes: progress.nodesProgress?.filter(n => n.status === 'completed').length || 0
      }
    })
  )

  // Get user_progress for problem solving stats
  const problemProgress = await db.collection('user_progress')
    .find({ user_id: userIdStr })
    .toArray()

  const completedProblems = problemProgress.filter(p => p.status === 'completed')
...
export async function getUserWeakTopics(userId) {
  ...
  // Calculate weak topics (< 70% correct, minimum 3 attempts)
  const weakTopics = Object.values(topicStats)
    .map(stat => ({
      topic: stat.topic,
      totalAttempts: stat.totalAttempts,
      correctAttempts: stat.correctAttempts,
      correctPercentage: Math.round((stat.correctAttempts / stat.totalAttempts) * 100)
    }))
    .filter(stat => stat.correctPercentage < 70 && stat.totalAttempts >= 3)
...
export async function getUserStudyPatterns(userId) {
  ...
  const mostActiveDays = Object.entries(byDay)
    .map(([day, count]) => ({ _id: parseInt(day), dayName: dayNames[day], count }))
    .sort((a, b) => b.count - a.count)
    .slice(0, 3)

  // Calculate average time spent on quizzes
  const totalTimeMinutes = quizResults.reduce((sum, q) => sum + (q.timeTaken || 0), 0)
  const averageSessionDuration = quizResults.length > 0
...
export async function getProgressTrends(userId, timeframeDays = 30) {
  ...
  const trends = Object.values(dailyData)
    .map(day => ({
      ...
      averageQuizScore: day.quizScores.length > 0
        ? Math.round(day.quizScores.reduce((sum, s) => sum + s, 0) / day.quizScores.length)
        : 0,
