...
function serializeDoc(doc) {
  ...
  for (const [key, value] of Object.entries(doc)) {
    if (value === null || value === undefined) {
      ...
      // ⭐ CHANGED: More robust check for nested objects
...
export async function getQuestionsByPattern(patternSlug) {
  try {
    ...
    console.log(
      `Found ${questions.length} questions for pattern ${patternSlug}`
    ); // Debug log
...
export async function initializeMasterProgress(
  userId,
  masterId,
  academicInfo = {}
) {
...
export async function isYearUnlocked(userId, masterId, yearNumber) {
  ...
  if (progress?.yearProgress && Array.isArray(progress.yearProgress)) {
    const prevYear = progress.yearProgress.find(
      (yp) => yp.year === yearNumber - 1
    );
    if (prevYear) {
  ...
  const previousYearProgress = await calculateYearProgress(
    userId,
    masterId,
    yearNumber - 1
  );
...
export async function updateMasterProgress(userId, masterId) {
  ...
  const updatedYearProgress = await Promise.all(
    master.years.map(async (year) => {
      const yearProg = progress.yearProgress.find(
        (yp) => yp.year === year.yearNumber
      );
      const completionPercent = await calculateYearProgress(
        userId,
        masterId,
        year.yearNumber
      );
      ...
      for (const slug of roadmapSlugs) {
        const roadmapProg = await getUserRoadmapProgress(
          userId.toString(),
          slug
        );
      ...
      return {
        ...
        completedAt:
          completionPercent === 100 ? new Date() : yearProg?.completedAt,
      };
    })
  );

  const currentYear =
    updatedYearProgress.findIndex((yp) => yp.completionPercent < 100) + 1 ||
    master.years.length;
...
export async function getYearTestQuestions(masterId, yearNumber, count = 10) {
  ...
  const allQuestions = await db
    .collection("year_test_questions")
...
export async function getMasterRoadmapAnalytics(masterId) {
  ...
  const techStackChoices = await db
    .collection("master_progress")
    .aggregate([
...
export async function getAcademicYears() {
  ...
  const years = await db
    ...
    .aggregate([{ $group: { _id: "$year" } }, { $sort: { _id: 1 } }])
    .toArray();
...
export async function deleteAcademicResource(
  ...
  await db
    .collection("academic_resources")
    .updateOne({ year, semester, subject }, { $pull: { resources: null } });
