  ...
  try {
    ...
    const questions = await Promise.all(
      jsonFiles.map(async (file, index) => {
        ...
        const optimalApproach = solution.approaches?.find(a => a.name === 'Optimal') || solution.approaches?.[0]
...
export async function getFullUserProfile(userId) {
  ...
  const profile = await db.collection("user_profiles").findOne({ userId: userId.toString() })
  const resume = await db.collection("user_resumes").findOne({ userId: userId.toString() })
