  ...
  useEffect(() => {
    if (selectedSubjectParam && subjects.length > 0) {
      const found = subjects.find(
        s => s.subject === decodeURIComponent(selectedSubjectParam)
      )
      if (found) {
        ...
        {selectedSubject ? (
          <div>
            ...
            {groupedResources.youtube.length > 0 && (
              <div className="mb-10">
                <h3 className="text-2xl font-bold mb-4">▶️ YouTube Playlists</h3>
                <div className="space-y-4">
                  {groupedResources.youtube.map((resource, idx) => (
                    <a
                      ...
                      className={`block p-4 rounded-lg border-2 ${resourceColors.youtube}`}
                    >
            ...
            {groupedResources.notes.length > 0 && (
              <div className="mb-10">
                <h3 className="text-2xl font-bold mb-4">📝 Handwritten Notes</h3>
                <div className="space-y-4">
                  {groupedResources.notes.map((resource, idx) => (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className={`block p-4 rounded-lg border-2 ${resourceColors.notes}`}
                    >
                      <div className="flex justify-between gap-4">
                        <h4 className="font-semibold">{resource.title}</h4>
            ...
            {groupedResources.article.length > 0 && (
              <div className="mb-10">
                <h3 className="text-2xl font-bold mb-4">📄 Articles & Guides</h3>
                <div className="space-y-4">
                  {groupedResources.article.map((resource, idx) => (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className={`block p-4 rounded-lg border-2 ${resourceColors.article}`}
                    >
                      <div className="flex justify-between gap-4">
                        <h4 className="font-semibold">{resource.title}</h4>
            ...
            {groupedResources.quantum.length > 0 && (
              <div className="mb-10">
                <h3 className="text-2xl font-bold mb-4">📖 Short Books & Quantum</h3>
                <div className="space-y-4">
                  {groupedResources.quantum.map((resource, idx) => (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className={`block p-4 rounded-lg border-2 ${resourceColors.quantum}`}
                    >
                      <div className="flex justify-between gap-4">
                        <h4 className="font-semibold">{resource.title}</h4>
          ...
          <div className="text-center py-12">
            ...
            <p>Select a subject from the list to view resources</p>
          </div>
        )}
      </div>
    </div>
