...
export default function ProgressSidebar({
  ...
  const completedNodes = userProgress?.nodesProgress?.filter(n => n.status === "completed").length || 0
    ...
    <div className="sticky top-24 space-y-4">
      <Card className="p-6 bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 dark:from-gray-800 dark:via-purple-900/20 dark:to-pink-900/20 border-2 border-blue-200 dark:border-blue-800">
        ...
        <div className="space-y-3 mb-4">
          {phaseBreakdown.map((phase, idx) => (
            <div key={idx} className="bg-white/50 dark:bg-gray-900/50 rounded-lg p-3 backdrop-blur">
              <div className="flex items-center justify-between text-sm mb-2">
                <span className="font-semibold">{phase.week}</span>
                <Badge variant={phase.percentage === 100 ? "default" : "outline"}>
        ...
        <div className="grid grid-cols-2 gap-3 pt-4 border-t">
          ...
          <div className="bg-white/50 dark:bg-gray-900/50 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-purple-600">{completedSubtopics}</div>
      ...
      {overallProgress < 100 ? (
        ...
        <Link href={`/roadmaps/${roadmap.slug}/quiz`}>
          <Card className="p-6 bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-950/30 dark:to-emerald-950/30 border-2 border-green-500 hover:shadow-lg transition-all cursor-pointer">
